...
if (isset($_POST['adminLogin'])) {
    ...
    $queryAdminLogin = "SELECT * FROM rjmtbl WHERE Username = '$username' AND role = 'admin'";
    $resultAdminLogin = mysqli_query($connection, $queryAdminLogin);
    $admin = mysqli_fetch_assoc($resultAdminLogin);

    if ($admin && $password === $admin['Password']) {
        // Admin login successful
        echo '<script>alert("Admin Login Successful! Welcome, ' . $admin['Username'] . '"); window.location.href="adminm.php";</script>';
    } else {
